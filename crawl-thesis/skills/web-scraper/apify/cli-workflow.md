...
flowchart LR
    R([my-scraper/])
    R --> A[.actor/]
    A --> A1["actor.json<br/><i>Actor configuration</i>"]
    A --> A2["input_schema.json<br/><i>Input validation</i>"]
    R --> S[src/]
    S --> S1["main.ts<br/><i>Your code here</i>"]
    R --> ST["storage/<br/><i>Local storage</i>"]
    R --> D["<b>config files</b><br/>.dockerignore<br/>.gitignore<br/>.prettierrc"]
    R --> AG["<b>AGENTS.md</b><br/><i>AI agent guidance (Apify-maintained)</i>"]
    R --> DF["Dockerfile<br/><i>Production build</i>"]
    R --> EL["eslint.config.js"]
    R --> PK["package.json<br/><i>Dependencies + scripts</i>"]
    R --> TS["tsconfig.json<br/><i>TypeScript config</i>"]
    R --> RM["README.md"]
