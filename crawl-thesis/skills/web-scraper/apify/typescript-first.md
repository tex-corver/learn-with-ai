...
flowchart LR
    R([my-scraper/])
    R --> S[src/]
    S --> S1["main.ts<br/><i>TypeScript source</i>"]
    S --> S2["types.ts<br/><i>Custom type definitions</i>"]
    R --> A[.actor/]
    A --> A1[actor.json]
    A --> A2[input_schema.json]
    R --> TS["tsconfig.json<br/><i>TypeScript config</i>"]
    R --> PK["package.json<br/><i>Build scripts</i>"]
    R --> DF[Dockerfile]
    R --> RM[README.md]
    style S1 fill:#d97757,color:#fff
    style S2 fill:#d97757,color:#fff
```
