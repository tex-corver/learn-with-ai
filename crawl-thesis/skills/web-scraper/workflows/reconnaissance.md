...
```mermaid
flowchart TD
    START([Reconnaissance Complete<br/>traffic captured])

    START --> API{"API discovered<br/>in traffic?"}
    API -- yes --> APIRoUTE["Prefer API route<br/><i>Phase 3: API strategy</i>"]
    APIRoUTE --> AUTH{"Auth required?"}
    AUTH -- no --> DIRECTAPI["<b>Direct API access</b><br/><i>fastest</i>"]
    AUTH -- yes --> RECORDAUTH["Record auth flow<br/>extract tokens"]
    API -- no --> DOM["DOM scraping needed"]
    DOM --> JS{"JavaScript-rendered?"}
    JS -- yes --> DEVTOOLS["DevTools bridge + humanizer"]
    JS -- no --> CHEERIO["<b>Cheerio</b><br/><i>10× faster</i>"]

    START --> PROT{"Protection detected?"}
    PROT -- Cloudflare<br/>bot detection --> STEALTH["Stealth mode handles<br/>browser detection"]
    STEALTH --> IP{"IP blocked?"}
    IP -- yes --> PROXY["Add upstream proxies"]
    PROT -- Rate limiting --> RL["Respect limits<br/>in implementation"]
    PROT -- CAPTCHA --> CAP["Consider CAPTCHA solver<br/>or manual intervention"]

    START --> SM{"Sitemap available?"}
    SM -- yes --> USEMAP["Use for URL discovery<br/><i>combine with API if found</i>"]
    SM -- no --> PAGN["Use traffic-discovered<br/>pagination or crawl"]

    style DIRECTAPI fill:#4ade80,color:#000
    style CHEERIO fill:#4ade80,color:#000
    style DEVTOOLS fill:#60a5fa,color:#000
    style PROXY fill:#fbbf24,color:#000
    style CAP fill:#f87171,color:#fff
```
