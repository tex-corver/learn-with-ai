...
flowchart LR
    ROOT([web-scraping/])
    ROOT --> SK["<b>SKILL.md</b><br/><i>main entry (proactive workflow)</i>"]
    ROOT --> WF[workflows/]
    WF --> WF1["reconnaissance.md<br/><i>Phase 1 interactive recon (CRITICAL)</i>"]
    WF --> WF2["implementation.md<br/><i>Phase 4 iterative impl</i>"]
    WF --> WF3["productionization.md<br/><i>Phase 5 Actor creation</i>"]
    ROOT --> ST[strategies/]
    ST --> ST1["framework-signatures.md"]
    ST --> ST2["cheerio-vs-browser-test.md"]
    ST --> ST3["proxy-escalation.md"]
    ST --> ST4["traffic-interception.md"]
    ST --> ST5["sitemap-discovery.md<br/><i>60× faster URLs</i>"]
    ST --> ST6["api-discovery.md<br/><i>10-100× faster</i>"]
    ST --> ST7["dom-scraping.md"]
    ST --> ST8["cheerio-scraping.md<br/><i>HTTP-only, 5× faster</i>"]
    ST --> ST9["hybrid-approaches.md"]
    ST --> ST10["anti-blocking.md"]
    ST --> ST11["session-workflows.md"]
    ROOT --> EX[examples/]
    EX --> EX1["*.js — runnable code"]
    ROOT --> RF[reference/]
    RF --> RF1["report-schema.md"]
    RF --> RF2["proxy-tool-reference.md"]
    RF --> RF3["regex-patterns.md / fingerprint-patterns.md"]
    RF --> RF4["anti-patterns.md"]
    ROOT --> AP[apify/]
    AP --> AP1["typescript-first.md"]
    AP --> AP2["cli-workflow.md"]
    AP --> AP3["templates/ examples/"]
    ROOT --> RM["README.md<br/><i>this file</i>"]
    style SK fill:#d97757,color:#fff
    style WF1 fill:#fbbf24,color:#000
```
